...
#include <windows.h> // waitable timer (CreateWaitableTimerExW etc.); pulls in synchapi.h
#endif
...
{
    // Sized exactly against the one layout allocated from this pool: a bindless
    // set per frame in flight (see
    // BGLBindlessDescriptorManager::createBindlessDescriptorSet). Each set
    // consumes, on top of the GLOBAL_DESCRIPTOR_COUNT-sized bindless array of
    // each type:
    //   storage buffers -- MATERIAL + SKIN + PALETTE, one single (non-array)
    //   buffer each image samplers  -- the 4 deferred G-buffer targets + one per
    //   shadow cascade
    ...
    constexpr uint32_t kShadowMaps =
        BGLBindlessDescriptorManager::SHADOW_MAP_CASCADE_COUNT;

    globalPool = BGLDescriptorPool::Builder(bglDevice)
                     .setMaxSets(kFrames)
                     .addPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                                  kFrames * GLOBAL_DESCRIPTOR_COUNT)
                     .addPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                                  kFrames * (GLOBAL_DESCRIPTOR_COUNT +
                                             kSingleStorageBuffers))
                     .addPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                                  kFrames * (GLOBAL_DESCRIPTOR_COUNT +
                                             kDeferredTargets + kShadowMaps))
                     .build();

    descriptorManager =
        std::make_unique<BGLBindlessDescriptorManager>(bglDevice, *globalPool);
    ...
    descriptorManager->writeDeferredRenderTargetToDescriptor(
        bglRenderer.getDRSampler(), bglRenderer.getDRDepthView(),
        bglRenderer.getDRNormalView(), bglRenderer.getDRAlbedoView(),
    ...
    initJolt();
    materialManager = std::make_unique<BGLMaterialManager>(bglDevice, *globalPool,
                                                           *descriptorManager);
...
Application::~Application()
{
    // Free all cached model geometry while the VkDevice is still alive. By now
    // the derived app's registry is destroyed, so every ModelComponent is already
    // gone — and since those own no GPU resources, the buffers live only here.
    // run()'s final vkDeviceWaitIdle left the GPU idle, so it's safe to destroy
    // them.
...
void Application::updateDirectionalUBO(entt::registry &registry, GlobalUBO &ubo,
                                       ...
                                       float aspect)
{
    ...
    if (!dirView.empty())
    {
        ...
        // cameraFovDegrees is HORIZONTAL; derive vertical from aspect (must match
        // the projection in run()).
        ...
        for (uint8_t ci = 0; ci < SHADOW_CASCADE_COUNT; ci++)
        {
            ...
            // Bounding sphere of the frustum slice: centre on the view axis at depth
            // cz, radius from whichever corner ring (near or far) is farther from it
            ...
            // Pull the light back so casters up to casterRange behind the slice still
            // render
        ...
        ubo.directionalLight.color =
            glm::vec4(dlc.color.x, dlc.color.y, dlc.color.z, dlc.lux);
...
void Application::run()
{
    VkDeviceSize uboAlignment =
        bglDevice.properties.limits.minUniformBufferOffsetAlignment;
    std::unique_ptr<BGLBuffer> uboBuffers = std::make_unique<BGLBuffer>(
        bglDevice, sizeof(GlobalUBO), BGLSwapChain::MAX_FRAMES_IN_FLIGHT,
        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
        uboAlignment);
    uboBuffers->map();

    std::unique_ptr<BGLBuffer> uboComposition = std::make_unique<BGLBuffer>(
        bglDevice, sizeof(CompositionUBO), 1, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
    uboComposition->map();

    std::array<VkDescriptorBufferInfo, BGLSwapChain::MAX_FRAMES_IN_FLIGHT>
        uboInfos;
    ...
    // SMAA precomputed LUTs (AreaTex/SearchTex) — consumed by the blending-weight
    // pass.
    ...
    GBufferRenderSystem gBufferRenderSystem{bglRenderer.getDeferredRenderPass(),
    ...
    CompositRenderSystem compositRenderSystem{
        bglRenderer.getCompositeRenderPass(), pipelineDescriptorSetLayouts,
        descriptorManager, registry};

    // SMAA pass 3 / present: blends composite + weights to the swapchain
    // (overlays draw after).
    SmaaNeighborhoodRenderSystem smaaNeighborhoodRenderSystem{
        bglRenderer.getSwapChainRenderPass(), pipelineDescriptorSetLayouts,
        descriptorManager};

    WireframeRenderSystem wireframeRenderSystem{
        bglRenderer.getSwapChainRenderPass(), pipelineDescriptorSetLayouts,
        descriptorManager, registry, bglDevice};

    // Bone-posing gizmo: interaction state + overlay renderer (drawn in the
    // swapchain pass).
    GizmoRenderSystem gizmoRenderSystem{bglRenderer.getSwapChainRenderPass(),
                                        pipelineDescriptorSetLayouts, bglDevice};
    // poseGizmo is an Application member (so the editmode console command can
    // reach it).

    PointLightSystem pointLightSystem{bglRenderer.getSwapChainRenderPass(),
                                      pipelineDescriptorSetLayouts,
                                      descriptorManager, registry, bglDevice};

    BloomRenderSystem bloomRenderSystem{bglRenderer.getBloomMipRenderPassClear(0),
                                        pipelineDescriptorSetLayouts,
                                        descriptorManager};

    RadiosityRenderSystem radiosityRenderSystem{
        bglRenderer.getRadiosityRenderPass(), pipelineDescriptorSetLayouts,
        descriptorManager};

    SmaaEdgeRenderSystem smaaEdgeRenderSystem{bglRenderer.getSmaaEdgeRenderPass(),
                                              pipelineDescriptorSetLayouts,
                                              descriptorManager};

    SmaaWeightRenderSystem smaaWeightRenderSystem{
        bglRenderer.getSmaaWeightRenderPass(), pipelineDescriptorSetLayouts,
        descriptorManager};

    AnimatedShadowRenderSystem animatedShadowRenderSystem{
        bglRenderer.getShadowMapRenderPass(), pipelineDescriptorSetLayouts,
        descriptorManager, registry};

    ShadowRenderSystem shadowRenderSystem{bglRenderer.getShadowMapRenderPass(),
                                          pipelineDescriptorSetLayouts,
                                          descriptorManager, registry};

    TransparentRenderSystem transparentRenderSystem{
        bglRenderer.getTransparentRenderPass(), pipelineDescriptorSetLayouts,
        descriptorManager, registry};

    // Procedural ocean. Same HDR pass as the transparents, drawn after them.
    WaterRenderSystem waterRenderSystem{bglRenderer.getTransparentRenderPass(),
                                        pipelineDescriptorSetLayouts, registry};

    {
        VkImageView
            shadowViews[BGLBindlessDescriptorManager::SHADOW_MAP_CASCADE_COUNT];
        for (uint32_t i = 0;
             i < BGLBindlessDescriptorManager::SHADOW_MAP_CASCADE_COUNT; i++)
            shadowViews[i] = bglRenderer.getShadowMapDepthView(i);
        descriptorManager->writeShadowMapToDescriptor(
            bglRenderer.getShadowMapSampler(), shadowViews);
    }
    ...
    // Profiling state (perf[]/sectMs[]/profAccum/profFrames + the Sect enum and
    // sectName[]) now lives on the Application as members so the extracted
    ...
    void *hFpsTimer = nullptr;
#ifdef _WIN32
    hFpsTimer = CreateWaitableTimerExW(nullptr, nullptr,
                                       CREATE_WAITABLE_TIMER_HIGH_RESOLUTION,
                                       TIMER_ALL_ACCESS);
    if (!hFpsTimer)
        hFpsTimer = CreateWaitableTimerW(nullptr, FALSE,
                                         nullptr); // fallback for older Windows 10
#endif
    ...
    // Let a derived app build its own render systems now that the swapchain pass
    // + layouts exist (drawn later via OnSwapchainOverlay). Keeps app-specific
    // rendering — e.g. LEGO — out of the engine loop.
    OnRenderInit(bglRenderer.getSwapChainRenderPass(),
                 pipelineDescriptorSetLayouts);

    auto frameLastTime = Clock::now();
    while (!bglWindow.shouldClose())
    {
        // Timestamp the start of this frame. frameTime = (this start - previous
        // start), and syncFrameRate() sleeps until this start + the target frame
        // duration. Without this per-frame update frameTime stays 0, so dt-scaled
        // movement (WASD) never advances.
        frameLastTime = Clock::now();
        // ` (grave) toggles all ImGui panels — kept HARD-CODED on purpose: it must
        // always work (never rebindable or clearable via `unbindall`) so you can't
        // lock yourself out of the UI/console. Edge-detected; ignored while an
        // ImGui text field is focused.
        {
            auto t0 = Clock::now();
            glfwPollEvents();

            bool graveDown = glfwGetKey(bglWindow.getGLFWWindow(),
                                        GLFW_KEY_GRAVE_ACCENT) == GLFW_PRESS;
            if (graveDown && !gravePrev && !ImGui::GetIO().WantTextInput)
                showImgui = !showImgui;
            gravePrev = graveDown;

            // Keybinds: fire each bound console command on its key's press edge
            // (Source-style `bind`). Suppressed while an ImGui text field is focused
            // so typing doesn't fire binds.
        ...
        // cameraFovDegrees is HORIZONTAL; derive vertical from the aspect ratio so
        // the horizontal framing stays fixed as the window/aspect changes.
        ...
        // Bone-posing gizmo input/logic (G toggles edit mode; W/E switch
        // translate/rotate).
        {
            t0 = Clock::now();
        ...
        {
            t0 = Clock::now();
            hierachy.ResolveSkeletonGlobals(); // resolve bones BEFORE parents so
...

        {
            ...
            if (runPhys)
            {
                ...
                BGLJolt::GetInstance()
                    ->ApplyGroupTransforms(); // followers ride their group body
            }
        ...
        {
            // Panels are built only when visible (toggled by `). NewFrame/Render
            // still run every frame so the ImGui backend stays balanced — it just
            // emits empty draw data. (drawImgui must be called every frame; it gates
            // the PANELS on showImgui internally. Gating the whole call would skip
            // Render(), freezing the last frame's panels on screen.)
            t0 = Clock::now();
        ...
        // this part caches the mat4 calculation on all transform components so
        // recalculation is unnecessary. this means render systems should never edit
        // the transforms
        cacheTransforms();
        ...
        if (primaryCommandBuffer)
        {
            ...
            compositRenderSystem.pushParams.bloomHandle =
                bloomEnabled ? bloomMipHandles[0] : 0u;
            ...
            if (ubo.hasDirLight)
            {
                ...
                for (uint32_t ci = 0; ci < SHADOW_CASCADE_COUNT; ci++)
                {
                    ...
                    shadowRenderSystem.renderShadowCasters(
                        frameInfo, ci, ubo.directionalLight.lightSpaceMatrix[ci]);
            ...
            // TODO(pre/post-water transparents): currently ALL transparents draw
            // before the water. For correct submerged-vs-surface transparency, split
            // the transparent queue into a pre-water group (here, before the water)
            // and a post-water group (after the water — atmosphere / glass over the
            // surface). See WaterRenderSystem.
            waterRenderSystem.renderEntities(frameInfo, gDepthHandle,
            ...
            if (bloomEnabled)
            {
                ...
                // Signed counter: a uint8_t `i >= 0` is always true, so the old loop
                // wrapped to 255 after the final iteration and indexed
                // bloomMips/bloomMipHandles out of range.
            ...
            smaaWeightRenderSystem.render(frameInfo, smaaEdgeHandle, smaaLuts.areaTex,
                                          smaaLuts.searchTex);
            bglRenderer.endCurrentRenderPass(primaryCommandBuffer);
            bglDevice.EndDebugUtilsLabel(primaryCommandBuffer);
            ...
            smaaNeighborhoodRenderSystem.render(frameInfo, compositeHandle,
                                                smaaWeightHandle,
                                                smaaEnabled && gbufferDebugMode == 0);

            if (showWireframe)
            {
                wireframeRenderSystem.renderModelsWireframe(
                    frameInfo); // planet + static models as wireframe
                wireframeRenderSystem.renderEntities(
                    frameInfo); // any WireframeComponent / collision overlay
            }
            if (drawBBox)
                wireframeRenderSystem.renderBBoxes(frameInfo);
            wireframeRenderSystem.renderSelection(
                frameInfo, selectedEntity); // amber outline on the picked block
            gizmoRenderSystem.render(frameInfo, poseGizmo);
            OnSwapchainOverlay(
                frameInfo); // app-specific overlays (e.g. LEGO connection markers)
...
void Application::syncFrameRate(
    void *timerHandle, std::chrono::steady_clock::time_point frameLastTime)
{
    if (maxFps > 0)
    {
        ...
        if (sleepMs > 0.5)
        {
            ...
            LARGE_INTEGER ft;
            ft.QuadPart =
                -(LONGLONG)(sleepMs * 10000.0); // 100-ns units, negative = relative
            SetWaitableTimer(timerHandle, &ft, 0, nullptr, nullptr, FALSE);
            WaitForSingleObject(timerHandle, INFINITE);
#else
            std::this_thread::sleep_for(
                std::chrono::microseconds((long long)(sleepMs * 1000.0)));
#endif
        }
    }
}
...
// Caller gates on the stutterDetect toggle; the threshold test + report live
// here (mirrors profile()). Reads the same per-frame sectMs[]/sectName[] the
// recordSection lambda fills.
void Application::detectStutter(double frameTime)
{
    ...
    printf("[STUTTER] %.1f ms (%.0f fps) | worst: %s %.3f ms\n",
           frameTime * 1000.0, 1.0 / frameTime, sectName[worst], sectMs[worst]);
}
...
{
    for (auto [animEnt, anim] :
         registry.view<AnimationPlaybackComponent>().each())
    {
        if (anim.manualPose)
        {
            auto &rig = registry.get<AnimationComponent>(animEnt);
            bool hasIK = false;
            for (const IKSetup &s : rig.ikSetups)
                if (s.valid())
                {
                    hasIK = true;
                    break;
            ...
            {
                // editPose + IK -> final pose (same helper the gizmo uses to place
                // markers).
                Pose finalPose;
                applyManualPose(rig.skeleton, rig.editPose, rig.ikSetups, finalPose);
                // Reuse the persistent scratch buffer: after it has grown once,
                // resize() to a size <= capacity does no allocation, so the per - frame
                // path stays alloc-free.
                paletteScratch.resize(anim.jointCount);
                resolvePalette(rig.skeleton, finalPose, paletteScratch.data());
                skinManager->writePalette(anim.dynamicPaletteBase,
                                          paletteScratch.data(), anim.jointCount);
                anim.poseDirty = false;
            }
            continue; // manual pose: skip clip playback for this entity
        }
        if (!anim.playing)
            continue;
        anim.time += frameTime;
        const float dur = anim.clipDuration();
...
void Application::registerDescriptorEntries()
{
    descriptorManager->writeDeferredRenderTargetToDescriptor(
        ...
        bglRenderer.getDRNormalView(), bglRenderer.getDRAlbedoView(),
        bglRenderer.getDREmissionView());
    // First call appends each render target and captures the bindless handle it
    // was assigned; later calls (after a gbuffer/window-resize recreate)
    // overwrite those same slots in place via the designated-handle path. The
    // handles aren't valid until the first append, so the initial registration
    // MUST use designated=false — otherwise storeTexture indexes a slot that
    // doesn't exist yet (vector subscript out of range).
    const bool reuse = renderTargetsRegistered;
    radiosityHandle = descriptorManager->storeTexture(
        bglRenderer.getRadiosityImageInfo(), bglRenderer.getRadiosityMemory(),
        bglRenderer.getRadiosityImage(), "RadiosityBuffer", reuse,
        radiosityHandle, false);
    // Opaque scene depth for the water pass. Not owned (the renderer owns the
    // G-buffer depth image); only sampled in the water pass where this image is
    // in DEPTH_STENCIL_READ_ONLY_OPTIMAL.
    gDepthHandle = descriptorManager->storeTexture(
        bglRenderer.getDRDepthImageInfo(), VK_NULL_HANDLE, VK_NULL_HANDLE,
        "GBufferDepth", reuse, gDepthHandle, false);
    smaaEdgeHandle = descriptorManager->storeTexture(
        bglRenderer.getSmaaEdgeImageInfo(), bglRenderer.getSmaaEdgeMemory(),
        bglRenderer.getSmaaEdgeImage(), "SmaaEdges", reuse, smaaEdgeHandle,
        false);
    smaaWeightHandle = descriptorManager->storeTexture(
        bglRenderer.getSmaaWeightImageInfo(), bglRenderer.getSmaaWeightMemory(),
        bglRenderer.getSmaaWeightImage(), "SmaaWeights", reuse, smaaWeightHandle,
        false);
    compositeHandle = descriptorManager->storeTexture(
        bglRenderer.getCompositeImageInfo(), bglRenderer.getCompositeMemory(),
        bglRenderer.getCompositeImage(), "CompositeLDR", reuse, compositeHandle,
        false);
    for (uint8_t i = 0; i < BGLRenderer::BLOOM_MIPS; i++)
    {
        bloomMipHandles[i] = descriptorManager->storeTexture(
            bglRenderer.getBloomMipImageInfo(i), bglRenderer.getBloomMipMemory(i),
            bglRenderer.getBloomMipImage(i), "BloomMip", reuse, bloomMipHandles[i],
            false);
    }
...
{
    // Re-register G-buffer, radiosity, and bloom mip descriptor entries after a
    // window resize
