...
class Application
{
  public:
    ...
    // The free-fly camera lives as an ECS entity so it reuses the TransformComponent
    // machinery. But Map::unload() clears the whole registry on every scene switch,
    // which destroys it — so run() can never hold a long-lived handle. This (re)creates
    // the viewer whenever it's missing, restoring the last pose, and is called at the top
    // of every frame before the transform is resolved.
    void ensureViewerEntity();
    ...
    void setGizmoEditMode(bool on)
    {
        poseGizmo.setEditMode(on);
    }
    bool gizmoEditModeOn() const
    {
        return poseGizmo.editModeOn();
    }

    // Source-style keybinds (key -> console command). Driven by the bind/unbind console
    // commands and polled each frame in run().
    KeyBindManager &getKeybinds()
    {
        return keybinds;
    }
    ...
    virtual std::string consoleLoadMap(const std::string &name)
    {
        return "[error] map: not supported in this app";
    }

    // Console "textmap <name>" hook: build /maps/<name>.yaml (human-readable static map) into the
    // live scene. Base is a no-op error; the derived app implements the actual load.
    virtual std::string consoleLoadTextMap(const std::string &name)
    {
        return "[error] textmap: not supported in this app";
    }
    ...
    virtual void OnSwapchainOverlay(FrameInfo &frameInfo)
    {
        (void)frameInfo;
    }
  ...
  protected:
    ...
    void setSpawnCameraRot(const glm::vec3 &r)
    {
        spawnCameraRot = r;
        spawnCameraRotDirty = true;
    }
    glm::vec3 spawnCameraRot{0.0f};
    bool spawnCameraRotDirty = false;
    // Free-fly camera entity + last pose, so ensureViewerEntity() can rebuild it (preserving
    // where the camera was looking) after a scene switch clears the registry. See run().
    entt::entity viewerEntity = entt::null;
    glm::vec3 viewerPosCache{0.0f, -3.0f, 0.0f};
    glm::vec3 viewerRotCache{0.0f};
