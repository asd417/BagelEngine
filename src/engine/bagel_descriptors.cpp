...
namespace bagel {
    ...
    bool BGLDescriptorPool::allocateDescriptor(
        const VkDescriptorSetLayout descriptorSetLayout, VkDescriptorSet& descriptor, uint32_t setCount) const {
        ...
        // No VkDescriptorSetVariableDescriptorCountAllocateInfo here: the bindless layout sizes every
        // array binding up front and sets no VARIABLE_DESCRIPTOR_COUNT_BIT, so a variable count would
        // be ignored by the spec and rejected by validation against the last binding (PALETTE, count 1).
    ...
    void BGLBindlessDescriptorManager::createBindlessDescriptorSet(uint16_t descriptorCount)
    {
        ...
        uint16_t targetIndex)
    {
        for (int i = 0; i < BGLSwapChain::MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            write.dstArrayElement = static_cast<uint16_t>(targetIndex);
    ...
    void BGLBindlessDescriptorManager::storeUBO(VkDescriptorBufferInfo bufferInfo, uint16_t targetIndex)
    {
    ...
    uint16_t BGLBindlessDescriptorManager::storeBuffer(VkDescriptorBufferInfo bufferInfo, const char* name = NULL)
    {
        uint16_t newHandle = static_cast<uint16_t>(buffers.size());
    ...
    void BGLBindlessDescriptorManager::rebindTextureSampler(uint16_t handle, VkSampler newSampler)
    {
    ...
    uint16_t BGLBindlessDescriptorManager::storeTexture(
        VkDescriptorImageInfo imageInfo,
        ...
        bool owned)
    {
        ...
        if (useDesignatedHandle) {
            ...
            handle = static_cast<uint16_t>(textures.size());
            TexturePackage pkg{};
    ...
    uint16_t BGLBindlessDescriptorManager::searchBufferName(std::string bufferName)
    {
        auto it = bufferIndexMap.find(bufferName);
        if (it == bufferIndexMap.end()) return std::numeric_limits<uint16_t>::max();
        return it->second;
    }

    uint16_t BGLBindlessDescriptorManager::searchTextureName(std::string textureName)
    {
        auto it = textureIndexMap.find(textureName);
        if (it == textureIndexMap.end()) return std::numeric_limits<uint16_t>::max();
        return it->second;
    }
    ...
    bool BGLBindlessDescriptorManager::checkMissingTexture(uint16_t index)
    {
