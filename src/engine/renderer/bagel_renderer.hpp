...
namespace bagel
{
	...
	{
	public:
		static constexpr uint8_t BLOOM_MIPS = 3;
		...
		void beginBloomDownsamplePass(VkCommandBuffer commandBuffer, uint8_t mip);
		void beginBloomUpsamplePass(VkCommandBuffer commandBuffer, uint8_t mip);
		VkRenderPass getBloomMipRenderPassClear(int mip) const { return bloomMips[mip].renderPassClear; }
		VkDescriptorImageInfo getBloomMipImageInfo(uint8_t mip) const
		{
			return {bloomMips[mip].sampler, bloomMips[mip].color.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
		}
		VkImage getBloomMipImage(uint8_t mip) const { return bloomMips[mip].color.image; }
		VkDeviceMemory getBloomMipMemory(uint8_t mip) const { return bloomMips[mip].color.mem; }
