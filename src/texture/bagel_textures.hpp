...
namespace bagel {
	...
	// Not inline: the definition lives in bagel_textures_load.cpp. Declaring it inline here
	// made every calling TU require a definition it never saw — lld-link rejects that.
	...
	public:
		...
		uint16_t loadTexture(const char* filePath, VkFormat imageFormat = VK_FORMAT_R8G8B8A8_SRGB);
		...
		uint16_t loadTextureFromMemory(const char* name, const uint8_t* rgba, uint16_t w, uint16_t h,
		...
		uint16_t updateTextureFromMemory(const char* name, const uint8_t* rgba, uint16_t w, uint16_t h,
		                                 VkFormat imageFormat = VK_FORMAT_R8G8B8A8_UNORM);
		...
		struct HostVisibleImage { uint16_t handle = 0; void* mapped = nullptr; size_t rowPitch = 0; bool ok = false; };
		HostVisibleImage createHostVisibleTexture(const char* name, uint16_t w, uint16_t h, VkFormat imageFormat);
		...
		uint16_t loadCombinedMetalRough(const char* roughPath, const char* metalPath);

	private:
		uint16_t buildAndStore(const char* filePath, VkFormat imageFormat);
		uint16_t buildAndStoreFromMemory(const char* name, VkFormat imageFormat, bool designatedIndex, uint16_t storedIndex);
		...
		void loadPixelDataInStagingBuffer(const uint8_t* rgba, uint16_t w, uint16_t h, uint8_t bpp = 4);
		...
		static uint16_t computeMipLevels(uint16_t w, uint16_t h);
	...
	private:
		...
		std::vector<uint16_t> contentTextureHandles{};
