...
namespace bagel {
	...
	uint16_t BGLTextureLoader::loadTexture(const char* filePath, VkFormat imageFormat)
	{
		return buildAndStore(filePath, imageFormat);
	}

	uint16_t BGLTextureLoader::buildAndStore(const char* filePath, VkFormat imageFormat)
	{
		uint16_t storedIndex = descriptorManager.searchTextureName(filePath);
		bool designatedIndex = false;
		if (storedIndex != std::numeric_limits<uint16_t>::max())
		{
			char buff[256];
			if (!descriptorManager.checkMissingTexture(storedIndex))
			{
				//sprintf_s(buff, "Texture %s is already bound", filePath);
				CONSOLE->Log("BGLTextureLoader", buff);
				return storedIndex;
			}
			//sprintf_s(buff, "Texture %s is bound but marked missing — reloading", filePath);
			CONSOLE->Log("BGLTextureLoader", buff);
	...
	uint16_t BGLTextureLoader::buildAndStoreFromMemory(const char* name, VkFormat imageFormat, bool designatedIndex, uint16_t storedIndex)
	{
		...
		uint16_t handle = descriptorManager.storeTexture(
	...
	uint16_t BGLTextureLoader::loadTextureFromMemory(const char* name, const uint8_t* rgba, uint16_t w, uint16_t h, VkFormat imageFormat)
	{
		uint16_t storedIndex = descriptorManager.searchTextureName(name);
		bool designatedIndex = false;
		if (storedIndex != std::numeric_limits<uint16_t>::max())
		{
	...
	uint16_t BGLTextureLoader::updateTextureFromMemory(const char* name, const uint8_t* rgba, uint16_t w, uint16_t h, VkFormat imageFormat)
	{
		uint16_t storedIndex = descriptorManager.searchTextureName(name);
		if (storedIndex == std::numeric_limits<uint16_t>::max())
	...
	BGLTextureLoader::HostVisibleImage BGLTextureLoader::createHostVisibleTexture(const char* name, uint16_t w, uint16_t h, VkFormat imageFormat)
	{
	...
	uint16_t BGLTextureLoader::computeMipLevels(uint16_t w, uint16_t h)
	{
	...
	void BGLTextureLoader::setMipLodBias(float bias)
	{
		...
		for (uint16_t handle : contentTextureHandles)
