...
namespace bagel
{
		...
		: BGLRenderSystem{renderPass, setLayouts, sizeof(WireframePushConstantData)}, device{bglDevice}
	{
		...
		std::vector<BGLModel::Vertex> marker = {
			...
			Tt,
			Bd,
			Tt, // cross-section -> pointy tail (+Y)
		};
	...
	void GizmoRenderSystem::render(FrameInfo &frameInfo, const PoseGizmo &gizmo)
	{
		...
		auto inSelection = [&](int j)
		{ return std::find(selSet.begin(), selSet.end(), j) != selSet.end(); };
		...
		for (size_t j = 0; j < jp.size(); ++j)
		{
			...
			if (p < 0 || p >= static_cast<int>(jp.size()))
				continue;
			glm::mat4 m{1.0f};
		...
		for (size_t j = 0; j < jp.size(); ++j)
		{
			...
			for (size_t c = 0; c < parents.size(); ++c)
				if (parents[c] == static_cast<int>(j))
				{
					boneLen = glm::length(jp[c] - jp[j]);
					hasChild = true;
					break;
				}
			...
			if (static_cast<int>(j) == sel)
				col = glm::vec4{1.0f, 0.6f, 0.0f, 1.0f};
			else if (inSelection(static_cast<int>(j)))
				col = glm::vec4{1.0f, 0.85f, 0.2f, 1.0f};
			if (disconnected)
