...
#include "entt.hpp"
#include "imgui/components/component_panels.hpp" // per-component registry-panel draw functions
...
inline void DrawInfoPanels(entt::registry &registry, uint16_t extentWidth, uint16_t extentHeight, glm::mat4 projectionMat, glm::mat4 viewMat)
{
    ...
    for (auto [ent, info, trans] : view.each())
    {
        ...
        float screenX = (normalizedDeviceCoordinate.x + 1.0f) * 0.5f * static_cast<float>(extentWidth);
        float screenY = (normalizedDeviceCoordinate.y + 1.0f) * 0.5f * static_cast<float>(extentHeight);
...
inline void DrawRegistryPanel(entt::registry &registry, PoseGizmo &gizmo)
{
    ...
    // hand-edited pose. The keyboard reference below mirrors PoseGizmo::update().
    {
        ...
        if (editing)
            ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.60f, 0.30f, 0.10f, 1.0f));
    ...
    { // live entities only (see count above)
        ...
        if (ImGui::TreeNode("ent", "Entity %u", idx))
        {
            ...
            DrawTransformComponent(registry, entity);
            DrawTransformArrayComponent(registry, entity);
            DrawTransformHierarchyComponent(registry, entity);
            DrawPointLightComponent(registry, entity);
            DrawDirectionalLightComponent(registry, entity);
            DrawModelComponent(registry, entity);
            DrawPlanetComponent(registry, entity);
            DrawAnimationComponent(registry, entity);
            DrawWireframeComponent(registry, entity);
            DrawDataBufferComponent(registry, entity);
            DrawJoltPhysicsComponent(registry, entity);
            DrawJoltKinematicComponent(registry, entity);
            DrawInfoComponent(registry, entity);

            ImGui::Separator();
            if (ImGui::SmallButton("Remove Entity"))
                toDelete = entity;
...
class ConsoleApp
{
  public:
    ...
    using CommandFn = const char *(*)(void *obj);
    using CommandArgFn = const char *(*)(void *obj, const char *args);
    ...
    bool Run(const char *command_line)
    {
