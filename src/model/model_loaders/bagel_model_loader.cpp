...
namespace bagel
{
	...
	void ModelLoaderBase::buildSkinBlock(const std::string &modelSourcePath)
	{
		...
		if (numSkins > 255)
			numSkins = 255; // ModelComponent::skinIndex / numSkins are uint8
	...
	std::vector<IKSetup> ModelLoaderBase::resolveIkSetups() const
	{
		...
		for (const ModelSidecar::IkChain &c : ikChains)
		{
			...
			{
				auto nm = [&](const std::string &n, int idx)
				{ return idx < 0 ? (" '" + n + "'?") : std::string(); };
	...
	std::vector<AttachmentComponent::Point> ModelLoaderBase::resolveAttachments() const
	{
		...
		if (attachmentDefs.empty() || skeleton.names.empty())
			return out;
		...
		for (const ModelSidecar::Attachment &d : attachmentDefs)
		{
            ...
            if (d.name.length()) {
				...
				size_t n = d.name.length();
				if (n > MAX_ATTACHMENT_NAME - 1)
				n = MAX_ATTACHMENT_NAME - 1;
				memset(p.name, 0, MAX_ATTACHMENT_NAME);
				memcpy(p.name, d.name.c_str(), n);
			}
			p.joint = joint;
