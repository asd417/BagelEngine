...
namespace bagel
{
	namespace
	{
		// glTF stores JOINTS_0 and WEIGHTS_0 as vec4 accessors, so influence `c` of vertex
		// `v` sits at flat index v*4 + c. Both attributes allow several component types;
		// the readers below are the only place that layout and those types are decoded.
		constexpr int INFLUENCES_PER_VERTEX = 4;

		template <typename T>
		T componentAt(const unsigned char* base, size_t v, int c)
		{
			return reinterpret_cast<const T*>(base)[v * INFLUENCES_PER_VERTEX + c];
		}

		// JOINTS_0 is ubyte or ushort — an index into the skeleton's joint palette.
		uint32_t readJoint(const unsigned char* base, size_t v, int c, int componentType)
		{
			return componentType == TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT
				? componentAt<uint16_t>(base, v, c)
				: componentAt<uint8_t>(base, v, c);
		}

		// WEIGHTS_0 is float, or a normalized integer whose full range maps onto [0,1].
		// An unknown component type yields 0; a vertex whose weights all vanish is pinned
		// to joint 0 by the caller, which beats reinterpreting unknown bytes as floats.
		// The casts are exact: every uint8_t and uint16_t is representable as a float.
		float readWeight(const unsigned char* base, size_t v, int c, int componentType)
		{
			switch (componentType)
			{
			case TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE:  return static_cast<float>(componentAt<uint8_t>(base, v, c)) / 255.0f;
			case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT: return static_cast<float>(componentAt<uint16_t>(base, v, c)) / 65535.0f;
			case TINYGLTF_COMPONENT_TYPE_FLOAT:          return componentAt<float>(base, v, c);
			default:                                     return 0.0f;
			}
		}

		// An animation sampler's accessor, viewed as a flat float array. glTF requires
		// sampler inputs/outputs to be FLOAT, so only the shape varies: `components` is
		// 1 for keyframe times, 3 for translation/scale, 4 for a rotation quaternion.
		...
		FloatAccessor floatAccessor(const tinygltf::Model& model, int accessor)
		{
			const tinygltf::Accessor&   acc  = model.accessors[accessor];
			const tinygltf::BufferView& view = model.bufferViews[acc.bufferView];
			FloatAccessor fa;
			fa.values     = reinterpret_cast<const float*>(&model.buffers[view.buffer].data[acc.byteOffset + view.byteOffset]);
			fa.count      = acc.count;
			fa.components = tinygltf::GetNumComponentsInType(acc.type);
			return fa;
		}

		// Channel `i` of a linear-RGBA baseColorFactor, quantized to 8-bit UNORM. A channel
		// the factor doesn't specify defaults to 1.0 (opaque white), matching the glTF default.
		uint8_t unorm8(const std::vector<double>& baseColorFactor, size_t i)
		{
			const double v = i < baseColorFactor.size() ? baseColorFactor[i] : 1.0;
			if (v <= 0.0) return 0;
			if (v >= 1.0) return 255;
			return static_cast<uint8_t>(v * 255.0 + 0.5);
		}

		// Untextured materials carry only a solid baseColorFactor. Bake it into a 1x1 UNORM
		// albedo texture so the existing albedo path shows the color. Deduped by value via
		// the generated name, so N materials sharing a color upload one texture.
		uint16_t solidColorAlbedo(BGLTextureLoader* pTextureLoader, const std::vector<double>& baseColorFactor)
		{
			if (!pTextureLoader) return 0;
			uint8_t px[4] = {
				unorm8(baseColorFactor, 0), unorm8(baseColorFactor, 1),
				unorm8(baseColorFactor, 2), unorm8(baseColorFactor, 3) };
			char name[32];
		...
		// A primitive with no material (index < 0) is opaque by definition.
		bool primIsTransparent(const tinygltf::Model& model, const tinygltf::Primitive& prim)
		{
			return prim.material >= 0
			    && prim.material < static_cast<int>(model.materials.size())
			    && GLTFModelLoader::isTransparent(model.materials[prim.material]);
		}
	}
	...
	void GLTFModelLoader::appendPrimitive(const tinygltf::Model& model, const tinygltf::Primitive& prim)
	{
		...
		if (jointsBuffer && weightsBuffer)
		{
			...
			for (size_t v = 0; v < vertexCount; v++)
			{
				...
				for (int c = 0; c < INFLUENCES_PER_VERTEX; c++)
				{
					// SkinInfluence stores joints as uint8_t; clamp rather than wrap.
					const uint32_t j = readJoint(jointsBuffer, v, c, jointsCompType);
					si.joints[c] = static_cast<uint8_t>(j > 255u ? 255u : j);
				}
				float w[INFLUENCES_PER_VERTEX];
				float sum = 0.0f;
				for (int c = 0; c < INFLUENCES_PER_VERTEX; c++) { w[c] = readWeight(weightsBuffer, v, c, weightsCompType); sum += w[c]; }
		...
		// Stays a lambda: `auto* buf` makes this a template, instantiated once per index
		// width by the switch below. A free function would have to be a function template
		// and take `indices`/`vertexStart` as parameters, which the capture gives for free.
	...
	void GLTFModelLoader::parseAnimations(const tinygltf::Model& model)
	{
		...
		for (const tinygltf::Animation& anim : model.animations)
		{
			...
			for (const tinygltf::AnimationSampler& s : anim.samplers)
			{
				...
				const FloatAccessor in = floatAccessor(model, s.input);
				as.times.assign(in.values, in.values + in.count);
				for (float tt : as.times) clip.duration = std::max(clip.duration, tt);

				const FloatAccessor out = floatAccessor(model, s.output);
				as.values.resize(out.count);
				for (size_t k = 0; k < out.count; k++)
				{
					glm::vec4 v(0.0f);
					for (int c = 0; c < out.components && c < 4; c++) v[c] = out.values[k * out.components + c];
					as.values[k] = v;
				}
	...
	void GLTFModelLoader::loadGLTFModel(const char* filename, bool mergeSolidSubmeshes)
	{
		...
		// Stays a lambda: it is a stateless callback whose only job is to report success
		// without decoding, and it exists solely as an argument to SetImageLoader.
		...
		for (size_t i = 0; i < model.materials.size(); i++)
		{
			...
			materials[i].albedoMap     = (pbr.baseColorTexture.index >= 0)
				...
				: solidColorAlbedo(pTextureLoader, pbr.baseColorFactor);
		...
		if (materials.empty())
		{
			...
			def.albedoMap = solidColorAlbedo(pTextureLoader, { 0.8, 0.8, 0.8, 1.0 });
		...
		if (mergeSolidSubmeshes)
		{
			...
			for (const tinygltf::Mesh& mesh : model.meshes)
				for (const tinygltf::Primitive& prim : mesh.primitives)
					if (!primIsTransparent(model, prim))
		...
		else
		{
			...
			for (const tinygltf::Mesh& mesh : model.meshes)
				for (const tinygltf::Primitive& prim : mesh.primitives)
				{
					if (primIsTransparent(model, prim)) continue;
		...
		for (const tinygltf::Mesh& mesh : model.meshes)
		{
			...
			for (const tinygltf::Primitive& prim : mesh.primitives)
				if (primIsTransparent(model, prim)) { hasTransparent = true; break; }
